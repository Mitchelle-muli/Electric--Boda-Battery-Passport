...
# app.py lives in frontend/; the backend package is a sibling directory.
# Add the project root so `backend.*` imports resolve regardless of the
# working directory `streamlit run` was launched from.
BASE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(BASE)
sys.path.insert(0, ROOT)

from backend.screen_fleet import screen_fleet, score_reading  # noqa: E402
from backend.live_simulator import (  # noqa: E402
    ALL_BATTERIES,
    SCAN_DAYS,
    generate_all_days,
    score_live_fleet,
)
from backend.notify import send_alert  # noqa: E402

DATA   = os.path.join(ROOT, "backend", "data", "Kore2_battery_performance.csv")
MODELS = os.path.join(ROOT, "backend", "models")

# Load models from backend/models/
def load_model():
    return joblib.load(os.path.join(MODELS, "rf_classifier.pkl"))

def load_kmeans():
    return joblib.load(os.path.join(MODELS, "kmeans_model.pkl"))
...
# ALL_BATTERIES and SCAN_DAYS come from backend.live_simulator (imported above)
# so the dashboard and the standalone simulator can never drift out of sync.
...
# run_screen, live-fleet simulation, single-reading scoring, and SMS
# alerts are NOT reimplemented here — they're imported from backend/ above
# (screen_fleet, score_live_fleet, score_reading, send_alert) so the
# dashboard and the standalone scripts can never silently drift apart,
# the way the two previously did (this file used to carry its own copy
# of each, and its own copy of send_alert used a different, hardcoded
# Twilio messaging_service_sid than backend/notify.py).
#
# screen_fleet() expects a `timestamp`-indexed DataFrame with a `state`
# column; wrap it with caching for the Streamlit UI:
run_screen = st.cache_data(show_spinner="Running fleet diagnostics ...")(screen_fleet)
